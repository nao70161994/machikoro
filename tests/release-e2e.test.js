...
function loadAppShellRuntime(profile = MOBILE_PROFILES[0]) {
    ...
    loadScripts(context, ['js/actionContract.js', 'js/clientStorage.js', 'js/appShellStorage.js', 'js/clientCheckpoint.js', 'js/clientReporting.js', 'js/clientReportingTransport.js', 'js/lifecycleNotify.js', 'js/lifecycleTransport.js', 'js/uiWatchdog.js', 'js/pwaShell.js', 'js/actionUiRegistry.js', 'js/uiTabView.js', 'js/appShell.js']);
...
function loadUiModalRuntime() {
    ...
    context.global = context;
    vm.createContext(context);
    loadScripts(context, ['js/Card.js', 'js/Player.js', 'js/clientStorage.js', 'js/uiLogDisplay.js', 'js/uiCardOrder.js', 'js/uiPendingMenu.js', 'js/uiTutorial.js', 'js/uiDiceChoice.js', 'js/uiModalPolicy.js', 'js/uiModalOpen.js', 'js/uiModalClose.js', 'js/uiWinner.js', 'js/uiGameStatusView.js', 'js/uiTabView.js', 'js/ui.js']);
...
runAsyncTest('release ntfy client-error-test は実送信せず mock fetch で通知内容を検証する', async () => {
    ...
    const ntfyUrl = new URL(calls[0].url);
    assert.strictEqual(ntfyUrl.origin + ntfyUrl.pathname, 'https://ntfy.sh/mock-topic');
    assert.strictEqual(ntfyUrl.searchParams.get('title'), '[ダイスシティ] Client Error');
