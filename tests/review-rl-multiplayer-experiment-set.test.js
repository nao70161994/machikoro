...
runTest('review-rl-multiplayer-experiment-set parseArgs は top-run-limit 0 を保持する', () => {
    const args = parseArgs(['--top-run-limit', '0']);
    assert.strictEqual(args.topRunLimit, 0);
});
...
runTest('review-rl-multiplayer-experiment-set buildReview/render は run 間順位表を出す', () => {
    ...
    try {
        ...
        const review = buildReview([inputA, inputB], { topRunLimit: 1 });
        const text = renderText(review);
        const markdown = renderMarkdown(review);
        assert.strictEqual(review.runs[0].runLabel, 'run-a');
        assert.strictEqual(review.recommendedRuns.length, 1);
        assert.strictEqual(review.diversifiedRuns.length, 1);
