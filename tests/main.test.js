...
runTest('広告 placeholder は許可された画面だけに配置される', () => {
    ...
    assert.ok(releaseChecklist.includes('no page script, form, button, `data-ui-action`, ad placeholder, or AdSense loader'));
    ...
    assert.ok(adsenseSetup.includes('remain static explanation pages without page scripts, forms, buttons, `data-ui-action`, ad placeholders, or an AdSense loader'));
    ...
    assert.ok(adsenseSetup.includes('grep -E "index,follow|privacy.html|アカウント登録なし|勝利条件|カード選択|保存と再開|最終更新日|og:description|twitter:description|og:image:alt|twitter:image:alt"'));
    assert.ok(adsenseSetup.includes('grep -E "index,follow|rules.html|アカウント登録|メールアドレス|エラー通知|Cookie|AdSense審査|Google AdSense|審査用スクリプト|実際の広告ユニット|お問い合わせ|最終更新日|og:description|twitter:description|og:image:alt|twitter:image:alt"'));
