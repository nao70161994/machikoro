...
runTest('広告 placeholder は許可された画面だけに配置される', () => {
    ...
    assert.ok(releaseChecklist.includes('no page script, form, button, app `id`/`data-*` attribute, `data-ui-action`, ad placeholder, or AdSense loader'));
    ...
    assert.ok(adsenseSetup.includes('remain static explanation pages without page scripts, forms, buttons, app `id`/`data-*` attributes, `data-ui-action`, ad placeholders, or an AdSense loader'));
