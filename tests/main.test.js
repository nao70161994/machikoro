...
runTest('広告 placeholder は許可された画面だけに配置される', () => {
    ...
    assert.ok(releaseChecklist.includes('Run the URL metadata / external connection hint checks, public OGP/PWA icon reachability checks, and static explanation page negative checks in `docs/ADSENSE_SETUP.md`'));
    ...
    assert.ok(adsenseSetup.includes('curl -fsS "$PUBLIC_ORIGIN/rules.html" | grep -E "index,follow|style.css|privacy.html|アカウント登録なし|勝利条件|カード選択|保存と再開|最終更新日|og:description|twitter:description|og:image|twitter:image|og:image:alt|twitter:image:alt"'));
