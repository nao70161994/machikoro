...
runTest('eval-expert-vs-weak parseArgs は expert-preset 値省略時も v2simple を使う', () => {
...
runTest('eval-expert-vs-weak formatter は主要値を含む', () => {
    ...
    assert.ok(text.includes('expertPreset=v2simple'));
    assert.ok(md.includes('- expertPreset: v2simple'));
    ...
    assert.ok(text.includes('businessMode=harmfulGift'));
    ...
    assert.ok(md.includes('- businessMode: harmfulGift'));
...
runTest('eval-expert-vs-weak fast pending は queue 先頭だけを解決する', () => {
