...
runTest('eval-expert-v2-benchmark-pack parseArgs は CLI 引数を解釈する', () => {
    const args = parseArgs(['--games', '3', '--seed', '7', '--max-steps', '900', '--format', 'markdown', '--full', '--expert-preset', 'default']);
...
runTest('eval-expert-v2-benchmark-pack evaluatePack は normal/strong の基準を返す', () => {
    const report = evaluatePack({ games: 1, seed: 1, maxSteps: 5000, lite: true, fast: false, expertPreset: 'v2simple' });
...
runTest('eval-expert-v2-benchmark-pack toText/toMarkdown は概要を出力する', () => {
    const report = {
        options: { games: 2, seed: 1, lite: true, fast: false, expertPreset: 'v2simple' },
        normal: {
