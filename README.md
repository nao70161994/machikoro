...
- AdSense 審査中の変更制限は [`docs/OPERATIONS.md`](./docs/OPERATIONS.md) の `AdSense Review Change Policy` を優先してください。審査中は docs / OGP文言 / 遊び方説明 / CI・test文書 / typo / static test hardening を中心に限定し、unknown通知修正、CI失敗修正、静的ページCSSは審査安定性を保つ緊急例外として扱います。UI大改修、広告位置変更、PWA挙動変更、URL変更、ルール変更、大規模リファクタ、実広告ユニット、SDK adapterは緊急対応以外では行いません。docs/static 変更でも最低限 `git diff --check`, `node tests/main.test.js`, `npm run test:static` を確認します。
