...
- 審査提出前と審査中の公開ページ再確認では、`docs/ADSENSE_SETUP.md` の URLメタ / 外部接続ヒント確認、OGP/PWA icon 到達確認、静的ページ負の確認を実行し、`Public page URL metadata and external connection hint checks passed` と `Static explanation page negative checks passed` で終わることを確認してください。
