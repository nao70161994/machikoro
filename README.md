...
- 審査提出前と審査中の公開ページ再確認では、`docs/ADSENSE_SETUP.md` の URLメタ / public-page link hint 確認、OGP/PWA icon 到達確認、ローカル寸法確認、静的ページ負の確認を実行し、`Public page URL metadata and public-page link hint checks passed`、`Local OGP/PWA icon dimension checks passed`、`Static explanation page negative checks passed` で終わることを確認してください。
