...
Current pre-review state must remain placeholder-only:

- No `pagead2.googlesyndication.com`, `adsbygoogle.js`, or `ca-pub-` script is loaded before AdSense approval.
...
- The subscribed Android/iPhone ntfy client receives a message with `phase=test`, `room=TEST01`, and `Machikoro ntfy test notification`.
...
## 6. AdSense Script Position After Approval

Only add the real AdSense loader after the site is approved or when Google explicitly asks for the review snippet.

Insert the loader in `index.html` inside `<head>`, after the existing PWA/meta/icon links and before `</head>`:
...
- Placeholder-only state is confirmed unless AdSense requires the review script snippet.
