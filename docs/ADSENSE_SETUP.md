...
- Confirm `privacy.html` and `rules.html` remain static explanation pages without page scripts, forms, buttons, embedded media elements, inline event handlers, app `id`/`data-*` attributes, `data-ui-action`, ad placeholders, or an AdSense loader.
