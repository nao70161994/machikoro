...
- `server/restoreGateway.js` owns only canonical-vs-client source selection and existing-room replace/reject/rejoin policy. `server/restoredRoom.js` owns only input-nonmutating host/sequence/hostless metadata planning and deterministic mutable room-shell assembly after validation and receives Snapshot sanitation as an injected dependency. Restore validation, audit verification, sanitation call order, Socket effects, mirror replay, and persistence ordering remain in `server.js`; do not move them as one batch.
