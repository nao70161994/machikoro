...
- `js/gameSnapshot.js` owns exact snapshot/undo/local-save serialization and shared hydrate mechanics. Local save generation and mutable restore delegate to it, while storage validation, legacy normalization, CPU recreation, UI effects, the `savedGame` key, and its unversioned JSON shape remain unchanged.
- `js/gameEngine.js` is the shared mutable action dispatcher. Client replay, server mirror, and local non-build CPU actions delegate to it. Rule-based CPU build selection returns a canonical proposal through `CPU.chooseBuildAction()`, while `CPUBuildExecution` and the existing online authority own application. `GameEngine.transitionSnapshot()` remains shadow-only; do not replace another live owner until hydrate adapter parity, authority, timing, multi-action parity, mixed-client negotiation, and rollback are separately covered.
...
- Client/lifecycle reporting, watchdog classification/diagnostic serialization, server payload/settings/restore sanitation, online payload/restore-rank/state observation, UI display/HTML helpers, and the injected PWA install controller are focused boundaries; keep network, socket, DOM recovery, reporting storage writes, lifecycle, modal, reconnect timing, and Service Worker update effects in their existing owners.
- CPU helper extraction must preserve wrapper results, the exact decision baseline, the 2–10 player/all-difficulty self-play baseline, fixed traces, and existing CPU tests. `CPUActionProposal.create()` owns canonical detached proposal shaping; `CPU.chooseBuildAction()` must not mutate game/stock, and `CPUBuildExecution.executeAction()` must apply at most one selected build. These boundaries do not authorize changing heuristic constants, difficulty presets, candidate order, RNG use, or online authority.
...
- Engine/Snapshot, canonical repository, reconnect shadow, and CPU action-proposal adapter boundaries have JSDoc contracts. Scoped ESLint is approved for the 44-module maintenance allowlist and must keep config/script parity. TypeScript/checkJs still require a separate dependency/config decision; do not infer runtime authority or turn either into repository-wide cleanup.
