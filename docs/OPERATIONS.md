...
- Client/server replay changes must preserve complete serialized snapshot parity for the same canonical action trace, including roll-generated multi-pending order, dormant-card exchange, Harbor/IT negative choices, and Tuna Boat dice retained through the Station → reroll → Harbor choice chain across 2/3/5/10 players and all v0/v1 Action/Snapshot selections.
