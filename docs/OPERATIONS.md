...
- Client/lifecycle reporting, shared client storage access, watchdog snapshot/phase/modal classification and diagnostic serialization, server payload/settings/restore sanitation, online payload/restore-rank/state observation, UI display/HTML helpers, and the injected PWA install controller are focused boundaries. Keep storage keys/value formats, network, socket, DOM/focus recovery effects, lifecycle timing, reconnect timing, and Service Worker update effects in their existing owners.
