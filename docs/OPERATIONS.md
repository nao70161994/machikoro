...
- `js/uiWatchdog.js` owns sorted freeze-issue dedupe signatures and ordered interactability-issue assembly from captured observations. Reporting dedupe must never suppress recovery; DOM reads, dedupe state, and recovery effects remain in `appShell.js`.
