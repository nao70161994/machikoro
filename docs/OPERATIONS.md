...
- `js/gameEngine.js` is the shared mutable action dispatcher. Client replay, server mirror, and local non-build CPU actions delegate to it; CPU build selection/execution and online authority retain their existing owners. `GameEngine.transitionSnapshot()` remains shadow-only; do not replace another live owner until hydrate adapter parity, authority, timing, multi-action parity, mixed-client negotiation, and rollback are separately covered.
