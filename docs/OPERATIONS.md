...
- `privacy.html` and `rules.html` are reachable from the title screen and cached by the PWA shell; the title page metadata mentions 登録不要 / no-registration play, and `privacy.html` also mentions contact guidance and the last updated date.
