...
- Action/Snapshot schema readers, `js/gameSchemaCodec.js`, and `GameEngine.transitionEnvelope()` accept legacy v0/current v1 internally, reject unknown or selection-mismatched inputs before hydration, and support independently selected Action/Snapshot versions in shadow results. Live traffic remains legacy/unversioned; never begin emitting v1 solely because these boundaries exist.
