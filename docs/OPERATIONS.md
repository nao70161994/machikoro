...
- `OnlineReconnectState` is a shadow controller with a pure event reducer and event-vs-legacy projection comparison, and `OnlineRetryPolicy` owns the existing 3s/8-attempt/15s calculations; neither authorizes changing timer/callback order, socket events, storage cleanup, or visible outcomes.
