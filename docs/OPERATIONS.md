...
- `js/gameSchemaNegotiation.js` is policy-only and production-unloaded. Missing peer capability means legacy v0; malformed explicit capability or no common version fails closed. Do not add capability fields or activate negotiated emission without a separate compatibility/rollback review.
