...
- Do not change during review unless an urgent incident or CI fix requires it: large UI redesigns, ad placement changes, live ad units, SDK adapters, PWA behavior changes, URL changes, rule changes, and broad refactors. Keep `privacy.html` and `rules.html` as static explanation pages without automatic redirects or meta refresh.
