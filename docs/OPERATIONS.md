...
- Adapter boundaries have JSDoc contracts. TypeScript 5.9.3 runs no-emit checkJs over the explicit 266-JavaScript browser/server allowlist, including `CPU.js`, `RLCPU.js`, `server.js`, and extracted runtime modules, via `npm run test:types` inside `test:static`; scoped ESLint covers the corresponding 267-file maintenance allowlist (including the Node-only Action Contract reporter). Concrete browser globals for Action, Snapshot, CPU proposal/build strategy, reconnect runtime, and online Engine runtime use `typeof import(...)` and must not be weakened to `unknown`. Keep both allowlists and their config tests synchronized, exclude the five remaining side-effect client composition roots (`appShell`, `main`, `online`, `storage`, `ui`), and do not infer runtime authority or repository-wide cleanup.
...
- `js/localResumeView.js` owns only the RL-preload button and local/online resume-section projection. Keep repository/session reads, storage keys/formats, and DOM writes in `storage.js`, and load/cache the view before it.
