...
- `js/uiRuntimeSnapshot.js` owns UI flow diagnostic projection, and `js/uiWatchdog.js` owns freeze-facts assembly from already captured observations. Keep live game/CPU/clock/DOM reads and every recovery/reporting effect in `ui.js`/`appShell.js`; preserve public wrappers and diagnostic field names.
