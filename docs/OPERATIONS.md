...
- `GAME_SCHEMA_RECREATE_WIRE_ENABLED=1` also requires negotiation and independently wraps the top-level `recreateRoom` request as `{schemaVersion:1,recreateRoom}`. The server still accepts unwrapped legacy requests and rejects unknown envelope versions before restore effects. This flag does not change embedded Snapshot/action-log formats or restore authority. Roll it back independently before disabling negotiation.
