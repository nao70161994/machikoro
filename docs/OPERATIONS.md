...
- Client/lifecycle reporting, app-shell storage access, watchdog classification/diagnostic serialization, server payload/settings/restore sanitation, online payload/restore-rank/state observation, UI display/HTML helpers, and the injected PWA install controller are focused boundaries; keep storage keys/value formats, network, socket, DOM recovery, lifecycle timing, modal, reconnect timing, and Service Worker update effects in their existing owners.
...
- Adapter boundaries have JSDoc contracts. TypeScript 5.9.3 runs no-emit checkJs over the explicit 83-module browser/server allowlist, including every extracted `server/*.js` module, via `npm run test:types` inside `test:static`; scoped ESLint remains approved for the separate 88-module maintenance allowlist. Keep both allowlists and their config tests synchronized, exclude giant runtime owners, and do not infer runtime authority or repository-wide cleanup.
