...
| Pure Game Engine live migration remains | Shared dispatch, hydration core, and shadow boundary complete; live cutover needs design | `js/gameEngine.js` owns client/server action dispatch, and `js/gameSnapshot.js` owns shared serialize/hydrate mechanics, but live owners still validate, authorize, schedule, and supply compatibility policies. | A premature cutover could change stock/undo adapters, callback order, online authority, or mixed-client behavior. | Expand representative multi-action shadow parity before any capability-gated live cutover. | Versioned wire emission and live pure-transition adoption require mixed-client rollout and rollback design. |
