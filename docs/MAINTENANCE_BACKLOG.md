...
3. Keep the 43-module scoped ESLint gate synchronized; introduce JSDoc/checkJs only as a separately approved dependency/config task.
...
- Safe non-device work completed: gameAction/disconnect handler extraction, event-labelled reconnect shadow observation, expert lookahead-gate and simulation-stock extraction, modal visibility/focus policy extraction, full player-panel HTML extraction, injected PWA install controller extraction, separately gated versioned live Action and rejoin/compacted-Snapshot wires, and shared local-save serialize/hydrate delegation with the exact legacy format retained.
