...
| `js/CPU.js` remains a giant mixed file | Design judgment required / future large task | Diagnostics, dice-frequency evaluation, pending resolution, cache, tuning, and player-count profile seams exist, but scoring, legal-move generation, and execution still interact closely. | CPU behavior changes can leak across live/selfplay/tests. | Require fixed traces plus representative benchmark parity before any further extraction. | CPU strength must not change; moving scoring/legal moves is no longer a trivial pure extraction. |
