...
| Schema action shadow | Negotiated versions had no evidence from the real accepted-action path before live cutover. | `GAME_SCHEMA_SHADOW_ENABLED=1` works only with negotiation enabled and compare-runs accepted server actions through the shared mirror adapter. Match/mismatch/error becomes internal diagnostics only and never blocks acceptance, ACK, broadcast, or persistence. The default remains off; a Socket.IO E2E fixes a matched v1 action. |
