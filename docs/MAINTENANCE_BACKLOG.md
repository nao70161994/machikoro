...
| CPU evaluation primitives | Pure dice frequency, score-cap, opponent-dilution, landmark-progress, combo, duplicate-purchase, economy-balance, and lookahead gate calculations lived inside the giant CPU class. | `js/cpuEvaluation.js` owns the unchanged calculations, including station/card-color dice-frequency aggregation and expert lookahead steps/eligibility; direct contracts, wrapper parity, decision snapshots, and full self-play regression guard behavior. |
...
| CPU action proposal | CPU decisions could pass mutable or non-canonical payloads directly into local/online execution. | `js/cpuActionProposal.js` validates every Action Contract payload variant, then returns a detached deeply frozen `{action, data}` proposal. Local non-build CPU actions now apply that proposal through the shared mutable `GameEngine`; online transport, build selection/execution, heuristic values, candidate order, and RNG remain unchanged. |
...
| Reconnect state observation | Reconnect phases had no shared names, event vocabulary, or bounded transition history. | `js/onlineReconnectState.js` freezes eight states, allowed transitions, and lifecycle events; `online.js` records reconnect/disconnect/restore/replay/activation/retry-exhausted/completed/reset in a diagnostic-only shadow controller. Existing booleans, timers, callbacks, status text, cleanup, and protocol remain authoritative and unchanged. |
...
| Shared snapshot serialization/hydration | Client, server mirror, and local save could construct and restore equivalent state through separate implementations. | `js/gameSnapshot.js` owns exact snapshot/undo/local-save serialization plus the shared mutable hydrate core. Local save generation and mutable restore now delegate to it while caller adapters preserve strict validation, legacy normalization, CPU recreation, UI effects, and the exact unversioned `savedGame` key/shape. |
| Shared action execution | Client replay, server mirror, and local CPU orchestration maintained overlapping action application paths. | `js/gameEngine.js` owns the shared mutable dispatch table with injected card, stock, and undo adapters. Client replay, server mirror, and local non-build CPU actions delegate to it; client/server validation, actor authority, inventory, timing, transport, and CPU build execution remain in their existing owners. |
...
| `js/CPU.js` remains a giant mixed file | Now safer, further movement needs design judgment | Diagnostics, evaluation primitives including card/station dice frequency and lookahead gates, base legal-move filters, weighted dice outcomes, lookahead loop/steps, pending resolution, cache, tuning, and player-count profiles now have seams. `js/cpuActionProposal.js` owns canonical detached action proposals, local non-build actions use the shared mutable Engine, and card/landmark build execution delegates to `js/cpuBuildExecution.js`. Large scoring/build-selection methods remain coupled. | Candidate or tie-break order changes can alter CPU strength; separating build selection from its current live `_buy*` calls can change candidate order or RNG consumption. | Keep extracting pure calculations with exact decision and full self-play parity; design a shadow action-only build strategy before changing live selection. | Broad scoring/selection movement and a live action-only build cutover remain deferred until shadow parity can prove identical decisions and RNG use. |
...
| JSDoc/checkJs/ESLint | Scoped typed boundaries and ESLint expanded | JSDoc now names the shared Engine mutable/transition adapters, Snapshot serialize/hydrate policies, canonical repository record/capabilities/methods, reconnect flags/controller/history, and CPU action proposals. ESLint covers 44 maintenance modules with bug-detection rules and config/script drift tests. | Add JSDoc with each newly touched adapter; introduce checkJs only as a separately reviewed dependency/config gate. | TypeScript is not installed. Whole-repository checkJs/lint or TypeScript migration would create broad churn and remains deferred. |
...
1. Preserve the current reconnect timer/callback ownership. Additional real-device online/PWA evidence is deferred because no further iPhone test device is available; do not infer it from automated WebKit.
2. Keep `npm run report:action-contract` issue-free and add behavior coverage before the next action/pending path change.
3. Keep the typed Engine/Snapshot/repository/reconnect/CPU-proposal JSDoc contracts and 44-module scoped ESLint gate synchronized; introduce checkJs only as a separately approved dependency/config task.
...
- Safe non-device work completed: gameAction/disconnect handler extraction, event-labelled reconnect shadow observation, expert lookahead-gate and simulation-stock extraction, modal visibility/focus policy extraction, full player-panel HTML extraction, injected PWA install controller extraction, separately gated versioned live Action and rejoin/compacted-Snapshot wires, shared local-save serialize/hydrate delegation with the exact legacy format retained, and typed Engine/Snapshot/repository/reconnect adapter boundaries.
