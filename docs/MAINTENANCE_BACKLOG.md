...
| Pure transition shadow | A future immutable engine cutover had no detached, fail-closed comparison seam. | `GameEngine.transitionSnapshot()` clones inputs, hydrates an isolated runtime, applies one shared action, and returns a detached snapshot with stable failure reasons. Real `GameManager` parity covers single actions plus 3-player build/Undo/next-turn, station dice selection, pending TV, and winning-landmark traces; no live path calls it yet. |
| Versioned engine transition footing | Action/Snapshot envelopes had no composed engine boundary for a future negotiated rollout. | `GameEngine.transitionEnvelope()` accepts legacy v0/current v1 Action and Snapshot envelopes, rejects unknown schemas before hydrate, and returns a v1 Snapshot envelope through the same detached transition. It remains shadow-only; live Socket.IO actions and saves retain their existing unversioned shapes. |
| Schema capability negotiation | Mixed old/new clients had no fail-closed pure rule for selecting Action/Snapshot versions. | `js/gameSchemaNegotiation.js` chooses the highest version common to every peer, treats missing capability as legacy v0, keeps Action/Snapshot versions independent, and rejects malformed explicit capability/no-common-version. It is not loaded by production or added to any live payload yet. |
...
- Extend detached shadow parity whenever a newly touched rule path is not represented by the current build/Undo, dice selection, pending TV, or victory traces.
