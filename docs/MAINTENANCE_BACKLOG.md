...
| JSDoc/checkJs/ESLint | Scoped typed boundaries and ESLint expanded | JSDoc now names the shared Engine mutable/transition adapters, Snapshot serialize/hydrate policies, canonical repository record/capabilities/methods, and reconnect flags/controller/history. ESLint covers 43 maintenance modules with bug-detection rules and config/script drift tests. | Add JSDoc with each newly touched adapter; introduce checkJs only as a separately reviewed dependency/config gate. | TypeScript is not installed. Whole-repository checkJs/lint or TypeScript migration would create broad churn and remains deferred. |
...
3. Keep the typed Engine/Snapshot/repository/reconnect JSDoc contracts and 43-module scoped ESLint gate synchronized; introduce checkJs only as a separately approved dependency/config task.
