...
| CPU evaluation primitives | Pure frequency, score-cap, opponent-dilution, and landmark-progress calculations lived inside the giant CPU class. | `js/cpuEvaluation.js` owns the unchanged calculations; direct contracts, wrapper parity, and CPU tests guard behavior. |
...
| CPU lookahead RNG | The deterministic playout generator lived beside lookahead orchestration and game cloning. | `js/cpuSimulation.js` owns the unchanged seeded LCG; sequence parity and the fixed seed/action trace guard behavior. |
...
| UI watchdog classification | Freeze-state classification was mixed with DOM snapshots, timers, recovery, and notifications. | `js/uiWatchdog.js` owns state-key/pending/classification decisions; `appShell.js` retains DOM reads, recovery, timing, reporting, and PWA behavior. |
...
| `js/CPU.js` remains a giant mixed file | Design judgment required / future large task | Diagnostics, evaluation primitives, deterministic playout RNG, pending resolution, cache, tuning, and player-count profile seams exist, but scoring, legal-move generation, simulation orchestration, and execution still interact closely. | CPU behavior changes can leak across live/selfplay/tests. | Require fixed traces plus representative benchmark parity before any further extraction. | CPU strength must not change; moving scoring/legal moves or simulation orchestration is no longer a trivial pure extraction. |
| `js/ui.js` still owns modal/stats/render orchestration | Now safer, still medium | Build/pending/detail/select/player/log/order helpers are split, but modal lifecycle, stats entry points, DOM mutation, focus/inert, and event processing remain in `ui.js`. | Selector drift is better guarded, but modal/focus/pointer regressions still need real devices. | Keep modal lifecycle stable until iPhone/Safari checks are possible. | No iPhone is available; large UI movement needs visual/mobile checks. |
