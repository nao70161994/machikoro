...
| Reconnect state observation | Reconnect phases had no shared names or testable transition vocabulary. | `js/onlineReconnectState.js` freezes eight states and allowed transitions; `online.js` exposes a read-only projection from existing booleans, including a distinct completed observation that resets to idle, without changing timers, callbacks, status text, or protocol. |
...
| Versioned live Action wire | Negotiated Action v1 needed a separately reversible transport step. | `js/gameSchemaWire.js` preserves exact legacy payload identity while OFF. `GAME_SCHEMA_WIRE_ENABLED=1` has effect only with negotiation enabled, envelopes v1 live `gameAction`/`actionAccepted`, keeps legacy rooms unversioned, preserves transport metadata, and rejects version mismatch before validation/replay. Socket.IO E2E covers v1, legacy fallback, malformed legacy-on-v1 rejection, and rejoin. Snapshot/save/rejoin bundle shapes remain unchanged. |
...
- Keep the default live wire legacy. The separately gated v1 Action wire may be exercised in automated/staged environments; Snapshot, save, and rejoin bundle payloads remain legacy until their own migration gates exist.
...
- Live pure-transition authority cutover and versioned Snapshot/save/rejoin rollout beyond the separately gated Action wire footing.
...
3. Keep the 42-module scoped ESLint gate synchronized; introduce JSDoc/checkJs only as a separately approved dependency/config task.
...
- Safe non-device work completed: gameAction/disconnect handler extraction, reconnect completed-state observation, expert lookahead-gate and simulation-stock extraction, modal visibility/focus policy extraction, injected PWA install controller extraction, and a separately gated versioned live Action wire.
