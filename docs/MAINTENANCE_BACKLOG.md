...
| UI pure render helpers | Build menu/card detail/card-select HTML generation lived inside `ui.js`, increasing escape and selector drift risk. | `js/uiBuildMenu.js`, `js/uiCardDetail.js`, and `js/uiCardSelect.js` provide pure HTML helpers with UI tests for escape/gate contracts. |
