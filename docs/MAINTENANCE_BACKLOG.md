...
| Socket handler families | Lobby, rejoin, and accepted-action sequencing were embedded directly in the server entrypoint. | `server/lobbySocketHandlers.js`, `server/rejoinSocketHandler.js`, and `server/actionSocketHandler.js` use dependency injection; tests lock registration and effect/emit order while event names and default payloads stay unchanged. |
...
| Item | Classification | Risk | Impact | Suggested action | Deferred reason |
| --- | --- | --- | --- | --- | --- |
| `server.js` socket handlers remain large | Partially migrated; further movement needs design | Lobby create/join, rejoin, and gameAction handlers now have dependency-injected families with exact order tests; recreate, disconnect, and HTTP wiring remain in `server.js`. | Remaining edits can still cross restore/disconnect boundaries. | Extend one handler family at a time only with event/effect-order contracts and online E2E. | recreate/disconnect movement changes restore/canonical/timing ownership and is deferred. |
...
- Remaining recreate/disconnect handler split using the established injected family seam.
...
## 2026-07-28 Current cycle decisions
...
- Safe non-device work completed: gameAction handler extraction, reconnect completed-state observation, expert lookahead-gate extraction, modal visibility/focus policy extraction, and a separately gated versioned live Action wire.
