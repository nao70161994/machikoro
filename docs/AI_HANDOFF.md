...
- CPU live option policy: `js/cpuTuning.js`がlive `expert` v2simpleのpreset/mode/tempo/airport既定値を入力非破壊で補う。`main.js`はCPU/RL生成を維持し、値変更時はdecision/self-play基準の意図的更新が必要。
...
- watchdog monitor: `js/uiWatchdog.js`がfreeze issueのfilter/sort/signatureをpureに所有し、`js/clientRuntimeSnapshot.js`がgroup化したgame/CPU/online/DOM factsを既存flat診断schemaへ投影し、`js/uiWatchdogMonitor.js`が5秒停滞判定と60秒の重複report抑止状態を所有する。`appShell.js`はDOM検査、復旧、保存、report effectを維持し、重複時も復旧は必ず試す。
- local resume policy: `js/localResumePolicy.js`がpending/no-save/invalid/RL preload/resume判定と保存済みCPU生成引数をpureに所有する。`storage.js`は既存key/schema、repository I/O、Promise、hydrate、DOM、scheduler effectを維持する。
...
- tooling: scoped ESLint 10.7.0は163個のmaintenance fileをbug-detection rulesだけで検査する。TypeScript 5.9.3のno-emit checkJsを162個のbrowser/server runtime（`CPU.js`、`RLCPU.js`、`server.js`、`server/*.js`全件を含む）へ限定導入し、`npm run test:types`を`test:static`へ統合済み。allowlistと残る5つのside-effect client runtime除外はcontract testで固定し、style rules、`--fix`、全repository lint/checkJs、TypeScript移行は禁止。
