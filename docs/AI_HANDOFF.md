...
- tooling: scoped ESLint 10.7.0は43個のmaintenance module（`uiPlayerDisplay.js`を含む）をbug-detection rulesだけで検査し、config/script driftもtestする。TypeScriptは未導入なのでcheckJsは別task。style rules、`--fix`、全repository lintは禁止。
