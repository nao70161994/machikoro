...
- app shell/storage: `js/clientReporting.js`、`js/lifecycleNotify.js`、`js/uiWatchdog.js` にreport、lifecycle payload/start/dedupeと注入clock/RNGからのsession ID生成、freeze分類、trace/root-cause整形、element可用性/lock理由、主要/pending/phase復旧可否、stale modal判定、保存用診断圧縮を分離。`js/clientStorage.js`を唯一の直接`localStorage` ownerとし、app shell/main/online/onlineStorage/storage/ui/statsはfacade経由へ移行済み。`js/appShellStorage.js`は既存契約の互換wrapper、`js/pwaShell.js`はinstall prompt/banner controllerを所有する。既存key/value/JSON形式、DOM snapshot/recovery、fetch、timer、SW更新副作用は不変。
