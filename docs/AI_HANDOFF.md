...
- contracts/engine: `js/actionContract.js`が15 actionのmetadata正本、`js/gameSnapshot.js`がclient/server/local-saveのserializeと共有hydrate mechanics、`js/gameEngine.js`が共有dispatchを所有する。全action・全Action/Snapshot v0/v1組合せparityを固定。`GAME_SCHEMA_WIRE_ENABLED`はlive Action、`GAME_SCHEMA_SNAPSHOT_WIRE_ENABLED`はrejoin responseと圧縮action metadataだけを独立した既定OFF gateでv1化できる。local `savedGame`は共有serializer/hydrateへ委譲済みだがkey/JSON形は不変。recreate-room input、restore logs、authority、timing、既定wireはlegacyのまま。
- tooling: scoped ESLint 10.7.0は43個のmaintenance moduleをbug-detection rulesだけで検査する。Engine mutable/transition、Snapshot serialize/hydrate、canonical repository、reconnect shadow controllerのadapter責務はJSDoc typedef化済み。TypeScript未導入のためcheckJsは別taskで、style rules、`--fix`、全repository lintは禁止。
