...
- watchdog monitor: `js/uiWatchdog.js`がfreeze issueのfilter/sort/signatureをpureに所有し、`js/uiWatchdogMonitor.js`が5秒停滞判定と60秒の重複report抑止状態を所有する。`appShell.js`はDOM検査、復旧、保存、report effectを維持し、重複時も復旧は必ず試す。
