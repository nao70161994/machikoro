...
- Lifecycle start/finish delivery now follows the same fail-visible contract: server delivery failure is `503`, while `js/lifecycleTransport.js` owns a compact maximum-8/7-day outbox and retries on startup/online recovery. Keep lifecycle payloads free of player names, room IDs, reconnect tokens, and snapshots.
