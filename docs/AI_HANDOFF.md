...
- online: 保存済み再接続sessionの必須field検証・空白除去・room ID正規化は`js/onlinePayload.js`へ統合済みで、`storage.js`はlocalStorage/UI effectを保持する。`js/onlineReconnectState.js`のshadow controllerは既存booleanから状態を投影するだけでなく、reconnect/disconnect/restore/replay/activation/retry-exhausted/completed/reset eventを履歴化する。pure event reducerとevent-vs-legacy projection契約は全eventおよびrestore/replay中切断を固定する。`js/onlineRetryPolicy.js`は既存3秒/8回/15秒契約を所有。既存boolean、Restore queues、timer callback、ACK timing、protocolが引き続きauthorityで、挙動は未変更。
