...
- tooling: scoped ESLint 10.7.0は74個のmaintenance moduleをbug-detection rulesだけで検査する。Engine mutable/transition、Snapshot serialize/hydrate、canonical repository、reconnect shadow controller、CPU action proposalのadapter責務はJSDoc typedef化済み。TypeScript未導入のためcheckJsは別taskで、style rules、`--fix`、全repository lintは禁止。
