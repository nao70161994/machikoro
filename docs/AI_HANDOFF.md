...
- client error入力: `js/clientReporting.js`がwindow error・unhandled rejection・console errorのreport入力をpureに組み立てる。`js/clientReportingTransport.js`がprivacy-reducedな最大8件/7日outbox、重複排除、bounded backoff、起動/online復旧時の再送、HTTP成功時だけの削除を所有し、`appShell.js`はhandler登録、console hook、crash表示とbrowser lifecycle配線を維持する。
...
## Fixed outcome 3 complete: online reconnect state runtime (2026-08-05)

- `js/onlineReconnectRuntime.js` owns the event controller, completion state, retry attempts, and rejoin timer. Keep lifecycle observation/reconcile/authority policy here rather than recreating it in `online.js`.
- Clean event history is default authority for reconnect state reads and the `isReconnectingOnline` compatibility projection. The two existing browser flags now act as explicit-false rollback controls; parity defects fail closed to legacy automatically.
- Start/disconnect/connect/rejoin/restore/replay/activation/failure/completion tests cover the promoted path. Socket event names/payloads, ACK/watermark, retry constants, restore queues, storage, and online Engine authority did not move.
- Fixed outcome 3 is complete. Next fixed scope is the separately default-OFF online shared-Engine authority with legacy fallback and client/server parity.


## Fixed outcome 4 complete: opt-in online shared Engine authority (2026-08-05)

- Online replay remains default-OFF. With both existing flags explicitly true, `OnlineGameEngineRuntime` adopts a successful detached transition before live mutation and skips duplicate replay.
...
## 2026-08-08 strong CPU stall handoff

- Start diagnosis with CPU execution checkpoints and the one-shot `cpu-step-abandoned` startup report. A hard Safari main-thread stall cannot run the watchdog until JavaScript yields; absence of an in-stall notification is not proof that reporting registration failed.
- Scheduler health must distinguish `timer pending` from `step executing`. Preserve execution ID/token/deadline matching so an old callback cannot clear or recover a newer step.
- Keep `CPUEvaluationCache.withStableSignature()` scoped to an immutable scoring operation. Do not promote it to a cross-turn cache without a complete mutation/invalidation contract.
- Strong post-roll resume is fixed by integration contracts for reroll, harbor, and build continuation. Preserve save shape and CPU decision/RNG behavior.
- `POST /api/client-error` success means ntfy accepted delivery. A missing topic, rejected status, thrown fetch, or invalid notifier result is `503` and remains queued by the browser.
- `GET /api/client-error-health` is the non-destructive readiness probe. Calling it against production needs the deployed allowed Origin or shared token and is intentionally not performed by repository tests.
- Lifecycle start/finish delivery now follows the same fail-visible contract: server delivery failure is `503`, while `js/lifecycleTransport.js` owns a compact maximum-8/7-day outbox and retries one entry at a time automatically using `Retry-After` plus bounded exponential backoff, including after startup/online recovery. Keep lifecycle payloads free of player names, room IDs, reconnect tokens, and snapshots.
- Remaining external checks are limited to the production health probe and optional real-iPhone reproduction. Do not block repository work on the unavailable iPhone device.
