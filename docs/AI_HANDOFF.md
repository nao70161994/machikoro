...
- tooling: scoped ESLint 10.7.0は88個のmaintenance moduleをbug-detection rulesだけで検査する。TypeScript 5.9.3のno-emit checkJsを73個のbrowser/server adapter（`server/*.js`は全件）へ限定導入し、`npm run test:types`を`test:static`へ統合済み。allowlistと巨大runtime除外はcontract testで固定し、style rules、`--fix`、全repository lint/checkJs、TypeScript移行は禁止。
