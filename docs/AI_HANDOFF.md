...
- app shell: `js/clientReporting.js`、`js/lifecycleNotify.js`、`js/uiWatchdog.js` にreport、lifecycle payload/start/dedupe、freeze分類、trace/root-cause整形、手番/online block判定、保存用診断圧縮を分離。`js/appShellStorage.js`に例外安全なstorage facade、`js/pwaShell.js`にinstall prompt/banner controllerを依存注入で分離。storage key/value形式、DOM snapshot/recovery、fetch、timer、SW更新副作用は既存ownerに残す。
...
- tooling: scoped ESLint 10.7.0は90個のmaintenance fileをbug-detection rulesだけで検査する。TypeScript 5.9.3のno-emit checkJsを89個のbrowser/server runtime（`CPU.js`、`RLCPU.js`、`server.js`、`server/*.js`全件を含む）へ限定導入し、`npm run test:types`を`test:static`へ統合済み。allowlistと残る5つのside-effect client runtime除外はcontract testで固定し、style rules、`--fix`、全repository lint/checkJs、TypeScript移行は禁止。
