...
- app shell: `js/clientReporting.js`、`js/lifecycleNotify.js`、`js/uiWatchdog.js` にreport、lifecycle payload/start/dedupe、freeze分類、trace/root-cause整形、手番/online block判定、保存用診断圧縮を分離。`js/clientStorage.js`に例外安全な共通storage facadeとprefix列挙、`js/appShellStorage.js`に既存契約の互換wrapper、`js/pwaShell.js`にinstall prompt/banner controllerを依存注入で分離。storage key/value形式、DOM snapshot/recovery、fetch、timer、SW更新副作用は既存ownerに残す。
...
- online: 保存済み再接続sessionの必須field検証・空白除去・room ID正規化は`js/onlinePayload.js`へ統合済みで、`storage.js`はlocalStorage/UI effectを保持する。`js/onlineReconnectState.js`はreconnect/disconnect/restore/replay/activation/retry-exhausted/completed/reset eventを履歴化し、pure reducerとlegacy projection契約を所有する。`ONLINE_RECONNECT_EVENT_AUTHORITY_ENABLED=1`時だけ、履歴が完全一致する場合にUI/send/CPU/human input gateがevent stateを読み、不一致・不正履歴では従来booleanへ戻る。開始→切断→再join→復元の統合parityを固定済み。`js/onlineRetryPolicy.js`は既存3秒/8回/15秒契約を所有。boolean write、Restore queues、timer callback、ACK timing、status、storage、protocolは引き続き既存authorityで、既定flag OFF時の挙動は未変更。
...
2026-07-18にAndroid 2台＋iPhone 2台の4人オンライン戦を再接続ありで勝利まで完走確認済み。これは基本同期と再接続継続の実機根拠だが、host移譲、server restart restore、Undo、online CPU、background復帰、PWA更新、modal focus/inertは未確認。2026-07-30時点で既定OFFのversioned live Action wireとrejoin/compacted-Snapshot wire、local-save serialize/hydrate共有化、event-labelled reconnect controllerとpure reducer/parity、既定OFF・fail-closedの入力/手番gate read authority、server reporting pure policy、local rule-based CPU buildの共有Engine適用、Business Center交換選択、app-shell storage、lifecycle開始/dedupeのpure分離まで実施済み。live pure-engine authority、recreate input/local-save version移行、reconnect timer/callback authority移行、recreate/restore authority、modal DOM副作用、CPUの大きなscoring/selectionはdeferred。production durable DB/providerは費用理由で保留し、追加iPhone実機確認も端末不在のため保留する。
