...
- `js/cpuSchedulerState.js` owns only CPU wait normalization, lease deadlines, pending-token identity, and scheduler-health projection. `main.js` retains the real clock, mutable token/deadline state, timers, checkpoints, phase ordering, and CPU action execution.
