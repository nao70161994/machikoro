...
5. **Current footing:** shared hydrate mechanics keep client/server legacy policy adapters explicit; detached server parity covers every Action Contract entry for all four independent Action/Snapshot v0/v1 selections, including build/Undo/next-turn, dice/reroll/harbor choices, every pending resolver, and victory. `js/gameSchemaCodec.js` enforces independently selected versions, and `transitionEnvelope()` preserves its prior behavior when no selection is supplied. `GAME_SCHEMA_NEGOTIATION_ENABLED` is an off-by-default rollback flag for additive capability transport. `GAME_SCHEMA_SHADOW_ENABLED` is a second off-by-default flag that, only when negotiation is also enabled, compare-runs accepted server actions through the negotiated shadow without affecting ACK, broadcast, persistence, or acceptance. **Future:** require production rollout/rollback design before any live Action/Snapshot cutover.
...
- `server/rejoinPayload.js` owns the injected snapshot/log/ACK/audit/provisional-field builder. `server/lobbySocketHandlers.js` and `server/rejoinSocketHandler.js` now own those handler families with exact effect-order tests; gameAction/recreate/disconnect timing remains in `server.js`.
...
- `GameEngine.transitionSnapshot()` is a detached, fail-closed shadow boundary with stable failure reasons. Real server-mirror parity covers all 15 actions under Action/Snapshot selections 0/0, 0/1, 1/0, and 1/1; it is not a live authority or transport path.
...
The remaining steps below still require the same gates described in each design section. In particular, live versioned wire emission/cutover, reconnect timer/callback migration, remaining gameAction/recreate/disconnect handler movement, modal DOM/focus/inert movement, and broad CPU scoring/selection movement need planned verification beyond current automated parity. The completed mixed Android/iPhone reconnect match is evidence for its exact path only; automated WebKit and that one match must not be recorded as completion of host migration, restart restore, provisional hostless timing, Undo, online CPU, background/PWA, or modal gates.
