...
| `js/uiBuildMenu.js` | Pure build gate/action-state projection, data-to-HTML helpers, safe button attributes, available-card display. |
...
- `server/gameLifecycleReporting.js` owns lifecycle notification payload/text formatting, `server/gameLifecycleGateway.js` owns auth/rate/normalize/dedupe/notify decision order and status, and `server/reportDelivery.js` owns injected ntfy delivery options; HTTP route/catch wiring remains in `server.js`.
- `server/clientErrorReporting.js`, `server/clientErrorAuth.js`, `server/reportingPolicy.js`, and `server/reportThrottle.js` own unchanged report shaping/classification/dedupe-key projection, request authorization, trust-proxy/topic/rate-key/debug-endpoint/dedupe policy, injected rate/dedupe algorithms, and per-report admission binding of limits/maps/dedupe keys. `server/clientErrorGateway.js` owns request decision order/status and `server/reportDelivery.js` owns injected ntfy option binding; routes and catch fallback remain in `server.js`.
...
- `js/clientReporting.js` also owns browser error, unhandled-rejection, and console-error input projection; `appShell.js` retains browser handler binding, console replacement, report transport, dedupe state, and crash-screen effects.
...
- `js/cpuEvaluation.js` also owns card-effect self-income evaluation behind a wrapper-parity contract. Its injected rule-income callback and constants preserve existing rule ownership, and fixed decision plus 2–10-player self-play baselines prohibit heuristic drift.
...
- `js/localResumePolicy.js` owns pending/no-save/invalid/RL-preload/resume decisions, detached runtime settings, unchanged saved-CPU construction arguments, and the exact resume effect order. `storage.js` retains repository reads, Promise handling, hydration, actual DOM/scheduler callbacks, keys, and formats.
