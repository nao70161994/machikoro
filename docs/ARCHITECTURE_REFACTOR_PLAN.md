...
- `js/gameSnapshot.js` owns exact current client/server snapshot and undo serialization plus shared mutable hydrate mechanics. Local `saveUndoState()` now delegates to the shared undo serializer with an explicit full-log compatibility limit. Client/server adapters deliberately retain their previous coin/index/log/landmark, inventory, and undo policies. Its legacy v0/current v1 envelope API is internal footing; live save and Socket.IO formats are unchanged.
