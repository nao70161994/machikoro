...
| Online client | `js/online.js`, `js/onlineComposition.js`, and online helpers | Socket lifecycle, retry/queue/session orchestration and action application remain in `online.js`; `OnlineComposition` binds session/game snapshots and storage/DOM/socket/client-effect adapters, while payload normalization/ACK comparison, restore rank, and state vocabulary have focused helpers. |
| UI | `js/ui.js` plus UI helpers | Top-level rendering, modal hide/focus/inert effects, DOM mutation, stats rendering, and event processing remain in `ui.js`; `UiCardSelectEffects` owns injected selection-view DOM application, while build/pending/detail/select/player/log/order/tutorial transforms, active-game status controls, tab/availability projection, pure modal keydown commands, and default-OFF modal-open plus post-hide modal-close plan/effect boundaries have focused helpers. |
