...
| UI | `js/ui.js` plus UI helpers | Top-level rendering, modal close/focus-trap/inert restoration, DOM mutation, stats rendering, and event processing remain in `ui.js`; build/pending/detail/select/player/log/order/tutorial transforms, active-game status controls, tab/availability projection, and a default-OFF modal-open plan/effect boundary have focused helpers. |
...
- `js/uiBuildMenu.js`, `js/uiPendingMenu.js`, `js/uiCardDetail.js`, `js/uiCardSelect.js`, `js/uiPlayerDisplay.js`, `js/uiLogDisplay.js`, `js/uiCardOrder.js`, `js/uiTutorial.js`, `js/uiWinner.js`, `js/uiDiceChoice.js`, `js/uiDiceDisplay.js`, `js/uiGameStatusView.js`, and `js/uiTabView.js` own pure HTML/display/order/guidance policy. `js/uiModalOpen.js` owns the exact modal-open plan and ordered effect executor behind a production-uninjected parity gate; inline legacy remains default/fallback and preserves focus-before-inert. `js/uiCardSelect.js` also owns input-nonmutating selection reducers and the sorted set/landmark view model; `js/uiGameStatusView.js` owns active-game status/control projection, `js/uiDiceDisplay.js` owns exact dice-face/result HTML plus opacity projection, and `js/uiTabView.js` owns tab plus availability projection; `ui.js`, `main.js`, and `appShell.js` still own DOM mutation, modal/timer effects, stats rendering, event handling, log history, and orchestration. Rolling preserves the prior dice opacity exactly.
...
2. **Further pure render helpers:** Move only exact-output helpers with escape and selector contract tests; do not enable the modal-open effect gate or move close/focus-trap/inert restoration yet.
