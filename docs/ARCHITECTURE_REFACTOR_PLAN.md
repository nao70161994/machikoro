...
- `js/onlineReconnectState.js` owns eight state names, allowed transitions, a pure lifecycle-event reducer, event-vs-legacy projection parity, and a bounded shadow controller. `js/onlineRetryPolicy.js` owns unchanged retry constants, exhaustion, deadline, and text calculations; timers and callbacks remain in `online.js`.
...
- Static runtime dependency tests guard extracted module load order across production, integration, release, online, UI, main, and self-play loaders. Scoped ESLint bug rules run from `test:static` over 87 maintenance modules, and a test keeps config and npm-script file sets identical.
