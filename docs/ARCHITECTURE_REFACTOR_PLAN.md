...
- `js/gameEngine.js` owns the shared 15-action mutable dispatch. Client replay, server mirror, and local non-build CPU actions delegate to it; client/server adapters still own validation, actor authority, card creation, stock mutation, undo restore, timing, and transport.
...
- `js/cpuEvaluation.js`, `js/cpuLegalMoves.js`, `js/cpuProfile.js`, `js/cpuSimulation.js`, `js/cpuActionProposal.js`, and `js/cpuBuildExecution.js` own unchanged evaluation, candidate, seeded simulation, 2–10-player lookahead stock construction, canonical detached Action Contract proposals, and local/online build execution. Local non-build proposals are applied through the shared mutable Engine. Rule-based build strategies now return one proposal through `CPU.chooseBuildAction()` before the execution adapter applies it; candidate order, RNG, and online authority remain unchanged.
...
- Static runtime dependency tests guard extracted module load order across production, integration, release, online, UI, main, and self-play loaders. Scoped ESLint bug rules run from `test:static` over 44 maintenance modules, and a test keeps config and npm-script file sets identical.
