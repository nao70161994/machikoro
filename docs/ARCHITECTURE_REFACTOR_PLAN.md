...
- Remaining handler moves and reconnect callback-side-effect/queue/status/cleanup-effect authority changes need careful sequencing; visible/socket timing changes still need external evidence even though the read, boolean, timer, and timeout-decision gates are automated and reversible.
