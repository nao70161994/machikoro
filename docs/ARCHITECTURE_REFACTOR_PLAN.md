...
| UI | `js/ui.js` plus UI helpers | Top-level rendering, modal/focus/inert, DOM mutation, stats rendering, and event processing remain in `ui.js`; build/pending/detail/select/player/log/order/tutorial transforms and tab view projection have pure helpers. |
...
| `js/uiTabView.js` | Pure top-level and online-tab display/class/ARIA projection plus the stats-render effect decision. |
...
- `js/uiBuildMenu.js`, `js/uiPendingMenu.js`, `js/uiCardDetail.js`, `js/uiCardSelect.js`, `js/uiPlayerDisplay.js`, `js/uiLogDisplay.js`, `js/uiCardOrder.js`, `js/uiTutorial.js`, `js/uiWinner.js`, `js/uiDiceChoice.js`, and `js/uiTabView.js` own pure HTML/display/order/guidance policy. `js/uiCardSelect.js` also owns input-nonmutating selection reducers and the sorted set/landmark view model; `js/uiTabView.js` owns top-level tab projection while `ui.js` still owns modal lifecycle, DOM mutation, stats rendering, event handling, log history, and render orchestration.
