...
- `js/cpuEvaluation.js`, `js/cpuLegalMoves.js`, `js/cpuProfile.js`, `js/cpuSimulation.js`, `js/cpuActionProposal.js`, and `js/cpuBuildExecution.js` own unchanged evaluation, candidate, seeded simulation, 2–10-player lookahead stock construction, canonical detached Action Contract proposals, and local/online build-execution boundaries behind existing CPU wrappers. Local non-build proposals are applied through the shared mutable Engine; build selection and online authority remain unchanged.
