...
- Static runtime dependency tests guard extracted module load order across production, integration, release, online, UI, main, and self-play loaders. Scoped ESLint bug rules run from `test:static` over 157 maintenance files, and a test keeps config and npm-script file sets identical.
