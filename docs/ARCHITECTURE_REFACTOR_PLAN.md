...
- `js/onlineReconnectState.js` owns eight state names, allowed transitions, a pure lifecycle-event reducer, event-vs-legacy projection parity, and a bounded event controller. Under the off-by-default `ONLINE_RECONNECT_EVENT_AUTHORITY_ENABLED` flag, only clean event history drives UI/send/CPU/human input gates; any mismatch or invalid event history falls back to the legacy boolean. Start/disconnect/connect/rejoin/restore integration parity is covered. `js/onlineRetryPolicy.js` owns unchanged retry calculations; boolean writes, timers, callbacks, queues, status text, storage, and protocol remain in `online.js`.
...
- `js/clientReporting.js`, `js/lifecycleNotify.js`, and `js/uiWatchdog.js` own pure report, lifecycle payload/start/dedupe, freeze classification, trace/root-cause transforms, turn/block gates, and bounded diagnostic serialization. `js/appShellStorage.js` owns injected exception-safe browser storage access, and `js/pwaShell.js` owns the injected install-prompt/banner controller. `appShell.js` retains browser capture, storage keys/value formats, DOM snapshots, recovery, fetch, timers, and public compatibility wrappers; Service Worker update wiring remains unchanged.
...
- `js/storageSettings.js` owns pure saved player-count, player-setting, and tutorial normalization. `storage.js` retains the existing localStorage keys, JSON I/O, DOM updates, exception boundary, and control synchronization.
...
- Static runtime dependency tests guard extracted module load order across production, integration, release, online, UI, main, and self-play loaders. Scoped ESLint bug rules run from `test:static` over 88 maintenance modules, and a test keeps config and npm-script file sets identical.
- JSDoc contracts for 73 explicitly listed browser/server adapters, including every extracted `server/*.js` module, are enforced by TypeScript 5.9.3 in no-emit checkJs mode through `npm run test:types`, which is part of `test:static`. Giant runtime owners remain excluded; this typed adapter gate does not imply live authority or a TypeScript migration.
