...
5. **Current footing:** shared hydrate mechanics keep client/server legacy policy adapters explicit; representative multi-action traces cover build/Undo/next-turn, station dice selection, pending TV, and victory. `js/gameSchemaCodec.js` enforces independently selected Action/Snapshot versions, and `transitionEnvelope()` preserves its prior behavior when no selection is supplied. `GAME_SCHEMA_NEGOTIATION_ENABLED` is an off-by-default rollback flag for additive capability transport. `GAME_SCHEMA_SHADOW_ENABLED` is a second off-by-default flag that, only when negotiation is also enabled, compare-runs accepted server actions through the negotiated shadow without affecting ACK, broadcast, persistence, or acceptance. **Future:** collect broad shadow evidence before any live Action/Snapshot cutover.
