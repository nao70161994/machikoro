...
| `js/onlineRestoreQueueState.js` | Input-nonmutating enqueue/overflow, rejoin-generation carry, and flush-start drain, and failed-index suffix-retention transitions; a production-uninjected flag selects them only on exact inline-legacy parity, while queue ownership, abort, handler execution, and callbacks remain in `online.js`. |
