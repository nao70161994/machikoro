...
- `js/cpuTuning.js` also owns the input-nonmutating live `expert` v2simple option projection. `main.js` retains CPU/RL construction, and exact decision plus 2–10-player self-play baselines remain the strength contract.
...
- `js/localResumePolicy.js` owns pending/no-save/invalid/RL-preload/resume decisions and the unchanged saved-CPU construction arguments. `storage.js` retains repository reads, Promise handling, hydration, DOM, scheduler effects, keys, and formats.
