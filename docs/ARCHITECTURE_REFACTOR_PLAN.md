...
| `js/onlineRestoreQueueState.js` | Input-nonmutating enqueue/overflow and rejoin-generation carry transitions; a production-uninjected flag selects them only on exact inline-legacy parity, while queue ownership, abort, flush, and callbacks remain in `online.js`. |
