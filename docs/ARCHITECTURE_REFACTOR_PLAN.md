...
- `js/onlinePayload.js` owns saved reconnect-session normalization, the existing rejoin payload shape, restore action-log/pending normalization, ACK comparison, room ownership, duplicate-free restore append, resend eligibility, and the pure restore-event flush plan with original queue indexes. An independent legacy plan remains default; a production-uninjected test-only flag selects the pure plan only after exact identity/index parity and fails closed to legacy. Queue storage, handler execution, failure recovery, reconnect timing, and Socket.IO ownership stay in `online.js`.
