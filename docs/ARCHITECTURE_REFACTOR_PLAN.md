...
- JSDoc contracts for 54 explicitly listed pure browser/server adapters are enforced by TypeScript 5.9.3 in no-emit checkJs mode through `npm run test:types`, which is part of `test:static`. Giant runtime owners remain excluded; this typed adapter gate does not imply live authority or a TypeScript migration.
