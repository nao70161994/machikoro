...
| `server/restoredRoom.js` | Input-nonmutating host/sequence/hostless metadata, new/replace/reject activation and canonical mirror-result planning, an opt-in/default-OFF ordered activation-effect executor with inline legacy fallback, plus deterministic mutable room-shell assembly from already-validated restore inputs with injected Snapshot sanitation/serialization; authority, replay, Socket, mirror, and persistence remain caller responsibilities. |
...
- `server/restoreValidation.js` owns game-start validation, undo-state sanitation, and restored human-slot reconstruction. `server/restoreGateway.js` owns canonical-vs-client source selection and existing-room replace/reject/rejoin decisions; `server/restoredRoom.js` owns validated restore metadata/activation/mirror-result planning, room-shell assembly, and a default-OFF detach/delete/install executor with inline legacy fallback. Validation/signing/sanitation, Socket effects, mirror, and persistence stay in their original order in `server.js`.
