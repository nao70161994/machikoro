...
5. **Current footing:** shared hydrate mechanics keep client/server legacy policy adapters explicit; representative multi-action traces cover build/Undo/next-turn, station dice selection, pending TV, and victory; `transitionEnvelope()` composes legacy/current schemas. `GAME_SCHEMA_NEGOTIATION_ENABLED` is an off-by-default rollback flag: when enabled, served clients advertise capabilities on create/join/rejoin and the server records the all-human common versions in additive `gameStart.gameSchema`; missing old clients force v0. **Future:** shadow-compare negotiated v1 encoding before any live Action/Snapshot cutover.
...
- `js/gameSchemaNegotiation.js` and `server/gameSchemaRuntime.js` define an off-by-default additive rollout boundary. With `GAME_SCHEMA_NEGOTIATION_ENABLED=1`, served clients advertise on create/join/rejoin, malformed or non-overlapping explicit capabilities fail closed, missing old clients select v0, and `gameStart.gameSchema` records the room result through normal/rejoin/restore paths. A real Socket.IO E2E covers v1, mixed-client v0 fallback, and incompatible rejoin rejection. Live Action/Snapshot payload encoding remains legacy even when enabled.
