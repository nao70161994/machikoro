...
- dice choice、card settings、business center chip selection は、入力状態や既存 DOM id 依存が強いため、別 PR で targeted test を先に追加してから移す。
- render 分割は、HTML 生成 helper を増やす小さい変更に限定する。
- custom modal 方針は維持し、native confirm は戻さない。

## 次に触る順序

1. dice choice の `data-action` 化。
2. business center chip selection の delegated handler 化。
3. card / landmark toggle の delegated handler 化。
