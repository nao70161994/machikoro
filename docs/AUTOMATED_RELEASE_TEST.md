...
| Service Worker update flow | Partial | `test:release` verifies `index.html` contains registration, `updatefound`, `controllerchange`, and `SKIP_WAITING`; it also dispatches SW `message` and `activate` events in vm. | Browser cache storage behavior across real upgrades and multi-tab controller timing remain manual only. |
