...
- Run the URL metadata / external connection hint checks, public OGP/PWA icon reachability checks, and static explanation page negative checks in `docs/ADSENSE_SETUP.md` before submitting and when rechecking public pages during review; they should finish with `Public page URL metadata and external connection hint checks passed` and `Static explanation page negative checks passed`.
