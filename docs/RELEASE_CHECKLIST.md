...
- Public pages include OGP/Twitter metadata with both `og:image` and `twitter:image` pointing to `/icons/icon-512.png`, plus image alt metadata; HTML title, OGP title, and Twitter title stay consistent and concise; the title page and rule-page metadata mention 登録不要 / no-registration play, and privacy-page metadata mentions error reporting / AdSense review / ad topics.
