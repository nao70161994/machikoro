...
- Public pages keep exactly one charset, one viewport, one HTML title, one `robots` meta with `index,follow`, and one shared `style.css` stylesheet in the head. Public pages include OGP/Twitter metadata with both `og:image` and `twitter:image` pointing to `/icons/icon-512.png` as same-origin relative paths, plus image alt metadata; `og:site_name`, `og:type`, and `twitter:card` stay stable; HTML title, OGP title, and Twitter title stay consistent and concise; public-page description / OGP / Twitter descriptions stay concise; public HTML metadata does not hardcode staging origins, localhost, or review-period external CSS hosts; the title page and rule-page metadata mention 登録不要 / no-registration play, and privacy-page metadata mentions error reporting / AdSense review / ad topics.
