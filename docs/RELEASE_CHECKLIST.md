...
- During AdSense review, do not add live ad units such as `<ins class="adsbygoogle">`, `data-ad-client`, `data-ad-slot`, or ad unit ids; the current review state is placeholder-only plus the single review loader.
...
- Run the URL metadata / external connection hint checks and static explanation page negative checks in `docs/ADSENSE_SETUP.md` before submitting; they should finish with `Public page URL metadata and external connection hint checks passed` and `Static explanation page negative checks passed`.
