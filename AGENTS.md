...
- 3人以上の RL 学習は `--player-count 3|4` で多人数用状態表現 (`STATE_DIM = 353`) を使います。現行の4人自己対戦プリセットは `sh scripts/rl/run-self-only-4p-h256-lr2e5-5000.sh` です。既存2人モデル (`STATE_DIM = 145`) との互換性は維持してください。
