...
- バックグラウンド運用では `sh scripts/rl/bg-watch-summary.sh <job>` で進捗要約、`sh scripts/rl/bg-experiment-set.sh <job>...` で比較セット確認、`sh scripts/rl/bg-finalize-top10-multiplayer.sh <job> 15 50` で完走後の多人数後評価まで一発で進められます。
